...
class DefaultDataSinkChecks {
    ...
    private void checkVideoOutputFormat(@NonNull MediaFormat format) {
        ...
        } else if (MediaFormatConstants.MIMETYPE_VIDEO_HEVC.equals(mime)) {
            LOG.i("Output video format is HEVC.");
        } else if (MediaFormatConstants.MIMETYPE_VIDEO_VP9.equals(mime)) {
            LOG.i("Output video format is VP9.");
        } else {
            throw new InvalidOutputFormatException("Video codecs other than AVC, HEVC, and VP9 are not supported, actual mime type: " + mime);
        }
    }
